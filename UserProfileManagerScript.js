...
document.getElementById("registrationForm").onsubmit = e => {
    ...
    if (password !== confirmPassword) {
        registerError.textContent = "Hasła muszą być identyczne!";
        return;
    }

    if (users.find(u => u.email === email)) {
        registerError.textContent = "Ten e-mail już istnieje!";
        return;
    }
...
function showCoursesPage(selectedId) {
    ...
    leaveBtn.textContent = "Wypisz się";
...
function registerForSeat(eventId, seatIndex) {
    ...
    if (seats.includes(currentUser.email))
        return alert("Już jesteś zapisany na ten kurs!");
